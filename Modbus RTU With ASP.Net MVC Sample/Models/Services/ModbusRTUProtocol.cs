...
namespace Modbus_RTU_With_ASP.Net_MVC_Sample.Models.WebClientApp.Models.Services
{
    public class ModbusRTUProtocol
    {
        ...
        private ushort startAddress = 164;
        ...
        public void Start()
        {
            try
            {
                ...
                serialPort1.Open();
                DBIO db = new DBIO();
                Table_Day sql_Data = new Table_Day();
                    ...
                    while (true)
                    {
                        if (serialPort1.IsOpen)
                        {
                            ...
                            if (serialPort1.BytesToRead >= 5)
                            {
                                ...
                                sql_Data.Time_Stamp = now;
                                sql_Data.Voltage_1 = result[0]*0.1;
                                sql_Data.Voltage_2 = result[2]*0.1;
                                db.AddObject(sql_Data);
